...
extension ChallengeDetailsView {
    ...
    class ViewModel {
        ...
        init(challenge: Challenge, modelContext: ModelContext, onNavigateBackToHomeScreen: @escaping () -> Void) {
        ...
        func markComplete() {
            let attemptRecord = ChallengeAttempt(startedAt: .now, status: .completed, challenge: challenge)
            ...
            if let progress = challenge.category.progress {
                ...
                showCycleCompleteSheet =  CycleManager.checkCycleCompletion(for: progress)
            }
        }
